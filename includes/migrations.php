  ...
  <tr><td align="center" style="padding:30px 12px;">
    <table role="presentation" width="600" cellpadding="0" cellspacing="0" style="width:100%;max-width:600px;border-collapse:collapse;background:#ffffff;">
      ...
      <tr><td align="center" style="background:#1a1a2e;padding:28px 32px;text-align:center;">
        ...
        [
            'version'     => 55,
            'description' => 'Add subscription type to citizens',
            'up' => function () {
                $cols = dbFetchAll("SHOW COLUMNS FROM citizens LIKE 'seminar_type'");
                if (empty($cols)) {
                    dbExecute("ALTER TABLE citizens ADD COLUMN seminar_type VARCHAR(255) NULL COMMENT 'Τύπος συνδρομής' AFTER last_name_lat");
                }
            },
        ],
        ...
        [
            'version'     => 63,
            'description' => 'Align custom role permissions with EasyRide modules',
            'up' => function () {
                $validSlugs = getAllPermissionSlugs();
                if (empty($validSlugs)) {
                    dbExecute("DELETE FROM custom_role_permissions");
                    return;
                }
        ...
        [
            'version'     => 64,
            'description' => 'Store Google Maps link for missions',
            'up' => function () {
                $col = dbFetchOne(
                    "SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS
                     WHERE TABLE_SCHEMA = DATABASE()
                       AND TABLE_NAME = 'missions'
                       AND COLUMN_NAME = 'maps_link'"
                );

                if (!$col) {
                    dbExecute("ALTER TABLE missions ADD COLUMN maps_link VARCHAR(1024) NULL AFTER location_details");
                }
            },
        ],
        ...
        [
            'version'     => 65,
            'description' => 'Store route points for mission map polylines',
            'up' => function () {
                $col = dbFetchOne(
                    "SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS
                     WHERE TABLE_SCHEMA = DATABASE()
                       AND TABLE_NAME = 'missions'
                       AND COLUMN_NAME = 'route_points'"
                );

                if (!$col) {
                    dbExecute("ALTER TABLE missions ADD COLUMN route_points MEDIUMTEXT NULL AFTER maps_link");
                }
            },
        ],
        ...
        [
            'version'     => 66,
            'description' => 'Create road partner locations table',
            'up' => function () {
                dbExecute("CREATE TABLE IF NOT EXISTS partner_locations (
                    id INT UNSIGNED AUTO_INCREMENT PRIMARY KEY,
                    ...
                    INDEX idx_partner_subscription (subscription_status)
                ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci");
            },
        ],
        ...
        [
            'version'     => 67,
            'description' => 'Link partner subscriptions to subscription records',
            'up' => function () {
                $hasSource = dbFetchOne(
                    "SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS
                     WHERE TABLE_SCHEMA = DATABASE()
                       AND TABLE_NAME = 'citizen_certificates'
                ...
                $hasPartnerIndex = dbFetchOne(
                    "SELECT INDEX_NAME FROM INFORMATION_SCHEMA.STATISTICS
                     WHERE TABLE_SCHEMA = DATABASE()
                       AND TABLE_NAME = 'citizen_certificates'
                       AND INDEX_NAME = 'idx_cc_partner'"
                );
                if (!$hasPartnerIndex) {
                    dbExecute("CREATE INDEX idx_cc_partner ON citizen_certificates(partner_id)");
                }

                $hasSourceIndex = dbFetchOne(
                    "SELECT INDEX_NAME FROM INFORMATION_SCHEMA.STATISTICS
                     WHERE TABLE_SCHEMA = DATABASE()
                       AND TABLE_NAME = 'citizen_certificates'
                       AND INDEX_NAME = 'idx_cc_source'"
                );
                if (!$hasSourceIndex) {
                    dbExecute("CREATE INDEX idx_cc_source ON citizen_certificates(source)");
                }
            },
        ],
        ...
        [
            'version'     => 68,
            'description' => 'Add duration months to subscription types',
            'up' => function () {
                $hasDuration = dbFetchOne(
                    "SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS
                     WHERE TABLE_SCHEMA = DATABASE()
                       AND TABLE_NAME = 'citizen_certificate_types'
                ...
                if (!$hasDuration) {
                    ...
                    dbExecute(
                        "UPDATE citizen_certificate_types
                         SET duration_months = 6
                         WHERE (LOWER(name) LIKE '%6%' OR name LIKE '%εξάμη%' OR name LIKE '%εξαμη%')
                           AND (name LIKE '%μην%' OR LOWER(name) LIKE '%month%')"
                    );
                    dbExecute(
                        "UPDATE citizen_certificate_types
                         SET duration_months = 3
                         WHERE (LOWER(name) LIKE '%3%' OR name LIKE '%τρίμη%' OR name LIKE '%τριμη%')
                           AND (name LIKE '%μην%' OR LOWER(name) LIKE '%month%')"
                    );
                    dbExecute(
                        "UPDATE citizen_certificate_types
                         SET duration_months = 1
                         WHERE (LOWER(name) LIKE '%1%' OR name LIKE '%μηνια%')
                           AND (name LIKE '%μην%' OR LOWER(name) LIKE '%month%')"
                    );
                }
            },
        ...
        [
            'version'     => 69,
            'description' => 'Extend ride mode GPS statuses and ping metadata',
            'up' => function () {
                $hasFieldStatus = dbFetchOne(
                    "SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS
                     WHERE TABLE_SCHEMA = DATABASE()
                       AND TABLE_NAME = 'participation_requests'
                       AND COLUMN_NAME = 'field_status'"
                ...
                if ($hasFieldStatus) {
                    dbExecute(
                        "ALTER TABLE participation_requests
                         MODIFY COLUMN field_status ENUM('on_way','on_site','needs_help','breakdown','left_behind','ok') NULL DEFAULT NULL"
                    );
                }
                ...
                $addColumn('accuracy', "accuracy DECIMAL(8,2) NULL AFTER lng");
                $addColumn('speed', "speed DECIMAL(8,2) NULL AFTER accuracy");
                $addColumn('heading', "heading DECIMAL(6,2) NULL AFTER speed");
                $addColumn('battery_level', "battery_level TINYINT UNSIGNED NULL AFTER heading");
                $addColumn('status', "status VARCHAR(30) NULL AFTER battery_level");
        ...
        [
            'version'     => 70,
            'description' => 'Create ride mode incident event log',
            'up' => function () {
                dbExecute(
                    "CREATE TABLE IF NOT EXISTS ride_events (
                        ...
                        user_id INT UNSIGNED NULL,
                        participation_id INT UNSIGNED NULL,
                        ...
                        created_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
                        INDEX idx_ride_events_mission_time (mission_id, created_at),
                        INDEX idx_ride_events_mission_active (mission_id, resolved_at, created_at),
                        INDEX idx_ride_events_user_time (user_id, created_at),
                        UNIQUE KEY uniq_ride_event_fingerprint (fingerprint),
                        CONSTRAINT fk_ride_events_mission FOREIGN KEY (mission_id) REFERENCES missions(id) ON DELETE CASCADE,
                        CONSTRAINT fk_ride_events_shift FOREIGN KEY (shift_id) REFERENCES shifts(id) ON DELETE SET NULL,
                        CONSTRAINT fk_ride_events_user FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE SET NULL,
                        CONSTRAINT fk_ride_events_participation FOREIGN KEY (participation_id) REFERENCES participation_requests(id) ON DELETE SET NULL,
                        CONSTRAINT fk_ride_events_resolved_by FOREIGN KEY (resolved_by) REFERENCES users(id) ON DELETE SET NULL
                    ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci"
                );
            },
        ],
        ...
        [
            'version'     => 71,
            'description' => 'Create ride start checklist items table',
            'up' => function () {
                dbExecute(
                    "CREATE TABLE IF NOT EXISTS ride_checklist_items (
                        id INT UNSIGNED AUTO_INCREMENT PRIMARY KEY,
                        mission_id INT UNSIGNED NOT NULL,
                        ...
                        updated_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
                        UNIQUE KEY uq_ride_checklist_mission_item (mission_id, item_key),
                        INDEX idx_ride_checklist_mission (mission_id),
                        CONSTRAINT fk_ride_checklist_mission FOREIGN KEY (mission_id) REFERENCES missions(id) ON DELETE CASCADE,
                        CONSTRAINT fk_ride_checklist_user FOREIGN KEY (completed_by) REFERENCES users(id) ON DELETE SET NULL
                    ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci"
                );
            },
        ],
