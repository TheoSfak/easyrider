...
?>
    ...
    <script>
        ...
        // Native <input type="date"> display is controlled by the browser/OS locale
        // (for example Chrome can show mm/dd/yyyy). Flatpickr gives us consistent
        // dd/mm/yyyy display while keeping yyyy-mm-dd submitted to PHP.
        flatpickr.localize(flatpickr.l10ns.gr);

        function initDateInput(el) {
            if (!el || el.dataset.voDatepickerReady === '1') return;
            el.dataset.voDatepickerReady = '1';
        ...
        function initDateInputs(root) {
            (root || document).querySelectorAll('input[type="date"], .datepicker').forEach(initDateInput);
        }

        function syncDateInputs(root) {
            (root || document).querySelectorAll('input[type="date"]').forEach(function(el) {
                if (el._flatpickr && el.value) {
                    el._flatpickr.setDate(el.value, false, 'Y-m-d');
        ...
        document.addEventListener('shown.bs.modal', function(event) {
            initDateInputs(event.target);
            syncDateInputs(event.target);
        });
        ...
        new MutationObserver(function(mutations) {
            mutations.forEach(function(mutation) {
                mutation.addedNodes.forEach(function(node) {
                    if (node.nodeType !== 1) return;
                    if (node.matches && (node.matches('input[type="date"]') || node.matches('.datepicker'))) {
                        initDateInput(node);
                    }
                    if (node.querySelectorAll) {
                        initDateInputs(node);
