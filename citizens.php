...
try {
    ...
    $seminarColumn = dbFetchOne("SHOW COLUMNS FROM citizens LIKE 'seminar_type'");
    if ($seminarColumn && preg_match('/varchar\((\d+)\)/i', $seminarColumn['Type'] ?? '', $m) && (int)$m[1] < 255) {
        dbExecute("ALTER TABLE citizens MODIFY COLUMN seminar_type VARCHAR(255) NULL COMMENT 'Τύπος συνδρομής'");
    }
} catch (Exception $e) {
    // Continue gracefully; the existing column is still usable.
}

$subscriptionTypes = dbFetchAll("SELECT name FROM citizen_certificate_types WHERE is_active = 1 ORDER BY name");
$subscriptionTypeNames = array_column($subscriptionTypes, 'name');
...
if (isPost()) {
    ...
    switch ($action) {
        ...
        case 'update':
            ...
            $subscriptionType = trim(post('seminar_type'));
            if ($subscriptionType !== '' && !in_array($subscriptionType, $subscriptionTypeNames, true)) {
                setFlash('error', 'Μη έγκυρος τύπος συνδρομής.');
                redirect('citizens.php');
            }
            $data = [
...
?>
                <thead class="table-light">
                    <tr>
                        ...
                        <th>Email / Τηλ.</th>
                        <th><?= sortLink('seminar_type',   'Συνδρομή',  $sortCol, $sortDir, $__f) ?></th>
                        <th>Γέννηση</th>
            ...
            <div class="modal-body">
                ...
                <div class="row g-3">
                    <div class="col-12">
                        ...
                        <select class="form-select" id="certModalTypeSelect" onchange="updateCertModalExpiry()">
                    ...
                    <div class="col-6">
                        ...
                        <input type="date" class="form-control" id="certModalExpiryDate" lang="el-GR" value="<?= date('Y-m-d', strtotime('+1 year')) ?>" readonly>
                        <div class="form-text">Υπολογίζεται αυτόματα από τη διάρκεια του τύπου συνδρομής.</div>
                    </div>
                </div>
            </div>
            <div class="modal-footer">
            ...
            <form method="post" id="citizenForm">
                ...
                <div class="modal-body">
                    <div class="row g-3">
                        ...
                        <div class="col-md-6">
                            <label class="form-label">Συνδρομή</label>
                            <select name="seminar_type" id="seminar_type" class="form-select">
                                <option value="">-- Επιλέξτε --</option>
                                <?php foreach ($subscriptionTypeNames as $type): ?>
...
var _certDefaultExpiry = '<?= date('Y-m-d', strtotime('+1 year')) ?>';
...
function updateCertModalExpiry() {
    var typeSelect = document.getElementById('certModalTypeSelect');
    var issueInput = document.getElementById('certModalIssueDate');
    var expiryInput = document.getElementById('certModalExpiryDate');
...
document.getElementById('certModalTypeSelect').addEventListener('change', updateCertModalExpiry);
document.getElementById('certModalIssueDate').addEventListener('change', updateCertModalExpiry);
                    ...
                    <div class="row g-2 align-items-end">
                        ...
                        <div class="col-md-4">
                            ...
                            <select id="chQuickNote" class="form-select form-select-sm">
                                ...
                                <option value="Να τον καλέσουμε για την επόμενη συνδρομή">Να τον καλέσουμε για την επόμενη συνδρομή</option>
