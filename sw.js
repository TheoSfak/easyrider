/**
 * EasyRide - Service Worker
 * Handles caching, offline fallback, and push notifications
 */

const CACHE_VERSION = 'er-v3.70.10-logo-icons';
...
self.addEventListener('activate', event => {
    event.waitUntil(
        caches.keys().then(keys =>
            Promise.all(
                keys.filter(key => (key.startsWith('vo-') || key.startsWith('er-')) && key !== STATIC_CACHE && key !== RUNTIME_CACHE)
...
self.addEventListener('push', event => {
    let data = { title: 'EasyRide', body: 'Νέα ειδοποίηση', icon: './assets/icons/icon-192.png' };
    ...
    const options = {
        ...
        tag: data.tag || 'easyride-notification',
        renotify: !!data.tag,
        data: {
