...
CREATE TABLE IF NOT EXISTS `users` (
    ...
    `driving_license` VARCHAR(30) NULL,
    `vehicle_plate` VARCHAR(20) NULL,
    `pants_size` VARCHAR(10) NULL,
    `shirt_size` VARCHAR(10) NULL,
    `blouse_size` VARCHAR(10) NULL,
    `fleece_size` VARCHAR(10) NULL,
    `registry_epidrasis` VARCHAR(50) NULL,
...
(1, 'Βόλτα', 'Οργανωμένη διαδρομή μελών', 'primary', 'bi-signpost-split', 1),
(2, 'Εκδρομή', 'Μεγαλύτερη διαδρομή ή πολυήμερη εξόρμηση', 'success', 'bi-map', 2),
(3, 'Συνάντηση', 'Συνάντηση λέσχης ή ενημέρωση μελών', 'info', 'bi-people', 3),
(4, 'Υποστήριξη', 'Δράση με ανάγκη συντονισμού και υποστήριξης', 'warning', 'bi-tools', 4),
(5, 'Εκδήλωση', 'Εκδήλωση, παρουσία ή συνεργασία λέσχης', 'purple', 'bi-calendar-event', 5),
(6, 'Training Ride', 'Εξάσκηση ομαδικής οδήγησης και road safety', 'teal', 'bi-arrow-repeat', 6);
...
CREATE TABLE IF NOT EXISTS `participation_requests` (
    ...
    `field_status` ENUM('on_way','on_site','needs_help','breakdown','left_behind','ok') NULL DEFAULT NULL,
...
('Λέσχη', 'Γενικές δράσεις και διαδρομές λέσχης'),
('Road Team', 'Συντονισμός διαδρομών και road captains'),
('Υποστήριξη', 'Βοήθεια, συνεργεία και σημεία υποστήριξης'),
('Εκδηλώσεις', 'Συναντήσεις, events και δημόσιες δράσεις');
...
('app_name', 'EasyRide'),
('app_description', 'Λέσχη Μοτοσικλετιστών'),
('points_per_hour', '10'),
...
CREATE TABLE IF NOT EXISTS `member_pings` (
    ...
    INDEX `idx_pings_user_time` (`user_id`, `created_at`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;

-- =============================================
-- RIDE MODE INCIDENT EVENTS
-- =============================================
...
CREATE TABLE IF NOT EXISTS `citizens` (
    ...
    `seminar_type` VARCHAR(255) NULL COMMENT 'Τύπος συνδρομής',
...
CREATE TABLE IF NOT EXISTS `citizen_certificates` (
    ...
    `source` VARCHAR(80) NULL COMMENT 'Πηγή εγγραφής',
    `partner_id` INT UNSIGNED NULL COMMENT 'Σύνδεση με συνεργάτη',
    `notes` TEXT NULL,
    ...
    INDEX `idx_cc_partner` (`partner_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
