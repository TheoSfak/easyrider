...
$rideDebriefSuggestion = buildRideDebriefSuggestion($id);
        ...
        <div class="col-lg-8 mx-auto">
            <div class="card shadow mb-4">
                ...
                <div class="card-body">
                    ...
                    <?php if (($rideDebriefSuggestion['event_count'] ?? 0) > 0 || ($rideDebriefSuggestion['ping_stats']['total_pings'] ?? 0) > 0): ?>
                    <div class="card border-primary mb-4">
                        <div class="card-header bg-primary bg-opacity-10 d-flex justify-content-between align-items-center">
                            <h6 class="mb-0 text-primary"><i class="bi bi-broadcast-pin me-1"></i>Πρόταση από Ride Mode</h6>
                            <span class="badge bg-light text-dark border">
                                <?= (int)$rideDebriefSuggestion['event_count'] ?> συμβάντα · <?= (int)$rideDebriefSuggestion['resolved_count'] ?> επιλυμένα
                            </span>
                        </div>
                        <div class="card-body">
                            <div class="mb-3">
                                <div class="small text-muted fw-semibold mb-1">Προτεινόμενη σύνοψη</div>
                                <pre class="bg-light border rounded p-2 mb-2 small" id="rideSummarySuggestion" style="white-space:pre-wrap;font-family:inherit;"><?= h($rideDebriefSuggestion['summary']) ?></pre>
                                <button type="button" class="btn btn-sm btn-outline-primary" data-fill-target="summary" data-source="rideSummarySuggestion">
                                    <i class="bi bi-arrow-down-square me-1"></i>Βάλε στη σύνοψη
                                </button>
                            </div>
                            <?php if (!empty($rideDebriefSuggestion['incidents'])): ?>
                            <div>
                                <div class="small text-muted fw-semibold mb-1">Προτεινόμενα συμβάντα</div>
                                <pre class="bg-light border rounded p-2 mb-2 small" id="rideIncidentsSuggestion" style="white-space:pre-wrap;font-family:inherit;"><?= h($rideDebriefSuggestion['incidents']) ?></pre>
                                <button type="button" class="btn btn-sm btn-outline-danger" data-fill-target="incidents" data-source="rideIncidentsSuggestion">
                                    <i class="bi bi-arrow-down-square me-1"></i>Βάλε στα συμβάντα
                                </button>
                            </div>
                            <?php endif; ?>
                        </div>
...
<script>
document.querySelectorAll('[data-fill-target][data-source]').forEach(function(button) {
    button.addEventListener('click', function() {
        var source = document.getElementById(button.getAttribute('data-source'));
        var target = document.getElementById(button.getAttribute('data-fill-target'));
