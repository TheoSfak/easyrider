...
if (isPost() && post('action') === 'duplicate' && isAdmin()) {
    ...
    setFlash('success', 'Η δράση αντιγράφηκε. Μπορείτε να την επεξεργαστείτε.');
        redirect('mission-form.php?id=' . $newId);
    }
    setFlash('error', 'Δεν βρέθηκε η δράση.');
...
// Dynamic page title
$statusTitles = [
    STATUS_OPEN => 'Ενεργές Δράσεις',
    STATUS_CLOSED => 'Κλειστές Δράσεις',
    STATUS_COMPLETED => 'Ολοκληρωμένες Δράσεις',
    STATUS_DRAFT => 'Πρόχειρες Δράσεις',
    STATUS_CANCELED => 'Ακυρωμένες Δράσεις'
];
$pageTitle = $statusTitles[$status] ?? 'Δράσεις';
...
<div class="card">
    <div class="card-body p-0">
        <?php if (empty($missions)): ?>
            <p class="text-muted text-center py-5">Δεν βρέθηκαν δράσεις.</p>
        <?php else: ?>
                            ...
                            <tr>
                                <td>
                                    ...
                                    <?php if ($mission['recurrence_id']): ?>
                                        <span class="badge bg-info text-dark ms-1" title="Δράση σε επαναλαμβανόμενη σειρά">
